...
function Admin() {
  ...
  useEffect(() => {
    axios.get('https://localhost:5000/api/users').then((response) => setUsers(response.data));
  }, []);

  const fetchAvailability = async (userId) => {
    try {
      const response = await axios.get(`//https://localhost:5000/api/availability/${userId}`);
